...
def main():
    ...
    with col_input:
        ...
        with st.container():
            ...
            with st.expander("⭐ Giả lập Đánh giá (Scenario Testing)"):
                ...
                critic_score = st.slider("Critic Score (Metacritic)", 0, 100, 0, help="Để 0 nếu game chưa phát hành")
                user_score = st.slider("User Score (IGDB/Steam)", 0, 100, 0, help="Để 0 nếu game chưa phát hành")
    ...
    if predict_btn:
        if not name.strip():
            st.error("⚠️ Vui lòng nhập tên game!")
            st.stop()
        if year > 2030:
            st.warning("⚠️ Năm phát hành quá xa (max 2030). Kết quả có thể không chính xác.")

        name_len = len(name)
        word_count = len(name.split())
        console_maker = get_console_maker(platform)
        comp_index = 800
        pub_exp = 3.0 if publisher in ui_data['publishers'] else 0.0
        ...
        predict_and_update_state(pipeline, input_df, raw_inputs, name, comp_index, "Ước tính")
            ...
            with tab_explain:
                try:
                    ...
                    try:
                        ...
                        critic_impact = vals[critic_idx]
                        user_impact = vals[user_idx]

                        st.write("### 📊 Tác động của đánh giá:")
                        k1, k2 = st.columns(2)
                        k1.metric("Critic Score Impact", f"{critic_impact:+.3f}",
                                  help="Giá trị dương (+) nghĩa là tăng khả năng HIT")
                        k2.metric("User Score Impact", f"{user_impact:+.3f}")
                    except ValueError:
                        pass
                    ...
                    ax.barh(range(len(indices)), vals[indices], color=colors)
                    ...
                    st.info(f"SHAP chưa khả dụng: {e}")
