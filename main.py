    ...
    with col_input:
        ...
        with st.container():
            ...
            with c_in1:
                ...
                plat_opts = sorted(df_raw['Platform'].unique())
                platform = st.selectbox("Hệ máy (Platform)", plat_opts)
                
                default_sequel = 0 if re.search(r'\d', name) else 1
                is_sequel_str = st.radio("Là phần tiếp theo?", ("Có", "Không"), index=default_sequel, horizontal=True)
                is_sequel = 1 if is_sequel_str == "Có" else 0
            with c_in2:
                pub_opts = sorted(df_raw['Publisher'].unique())
                publisher = st.selectbox("Nhà phát hành", pub_opts)
                genre_opts = sorted(df_raw['Genre'].unique())
    ...
    if predict_btn:
        ...
        with col_output:
            ...
            with tab_explain:
                ...
                with st.spinner("Đang phân tích SHAP..."):
